...
export function FilterDrawer({ activeCount, basePath = "/shop", lockedType, lockedCategory }: { activeCount: number; basePath?: string; lockedType?: string; lockedCategory?: string }) {
  ...
  return (
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid h-auto max-h-full w-full place-items-start overflow-y-auto bg-black/50 p-4" role="dialog" aria-modal="true" aria-label="Filters" onClick={() => setOpen(false)}>
          <div className="w-full max-w-2xl rounded-3xl bg-[#f9f5f0] p-1 shadow-xl" onClick={e => e.stopPropagation()}>
